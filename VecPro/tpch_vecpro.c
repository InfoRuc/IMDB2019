...
double SIMD_O4_test(data_t *l_tax, data_t *l_extendedprice, data_t *l_quantity,int columnlength,int vector_len);
/* $begin mountainmain */
int main()
{
    ...
    printf("New SIMD vectorized processing performance(cycle-per-tuple):\n");
    vec_len=1;finalresult=0;
    for(i=0;i<=MAXVECSTEP;i++){
        startTimer(&timer);
        finalresult=SIMD_O4_test(l_tax,l_extendedprice,l_quantity,MAXELEMS,vec_len);  /*warm up*/
        stopTimer(&timer);
        cycles = (double)timer/MAXELEMS;
        printf("vector length:%d result:%lf cycles:%4.2f\n", vec_len,finalresult,cycles);
        vec_len=vec_len*2;
    ...
    printf("New SIMD processing performance(cycle-per-tuple):\n");
    vec_len=1;finalresult=0;
    startTimer(&timer);
    finalresult=SIMD_O4_test(l_tax,l_extendedprice,l_quantity,MAXELEMS,MAXELEMS);  /*warm up*/
    stopTimer(&timer);
    cycles = (double)timer/MAXELEMS;
    printf("vector length:%d result:%lf cycles:%4.2f\n", MAXELEMS,finalresult,cycles);
...
double SIMD_O4_test(data_t *l_tax, data_t *l_extendedprice, data_t *l_quantity,int columnlength,int vector_len)
{
    ...
    size_t cntBlock = vector_len / nBlockWidth;    //向量中SIMD块数.
    size_t cntRem = vector_len % nBlockWidth;    //剩余数量.
    __m256 *yfsLoad1=(__m256 *)malloc(sizeof(__m256) * 4);    //加载extendedprice向量.
    __m256 *yfsLoad2=(__m256 *)malloc(sizeof(__m256) * 4);    //加载quantity向量.
    __m256 *yfsLoad3=(__m256 *)malloc(sizeof(__m256) * 4);    //加载tax向量.
    __m256 *yfsLoad4=(__m256 *)malloc(sizeof(__m256) * 4);    //加载1+tax向量.
    const data_t* p1 = l_extendedprice;    // AVX批量处理时所用的指针.
    const data_t* p2 = l_quantity;    // AVX批量处理时所用的指针.
    const data_t* p3 = l_tax;    // AVX批量处理时所用的指针.
    data_t* p4 = netto_value;    // AVX批量处理时所用的指针.
    ...
    __m256 *yfsSum1 = (__m256 *)malloc(sizeof(__m256) * 4);    //l_extendedprice*l_quantity临时结果变量。
    __m256 *yfsSum2 = (__m256 *)malloc(sizeof(__m256) * 4);    //1-l_tax临时结果变量。
    __m256 *yfsSum3 = (__m256 *)malloc(sizeof(__m256) * 4);    //聚集计算结果变量。[AVX] 赋初值0
    for (int i = 0; i < 4; i++) {
        yfsSum1[i] = _mm256_setzero_ps();    //[AVX] 赋初值0
        yfsSum2[i] = _mm256_setzero_ps();    //[AVX] 赋初值0
        yfsSum3[i] = _mm256_setzero_ps();    //[AVX] 赋初值0
    }
    __m256 yfsSum_plus_1 = _mm256_set1_ps(1);  // [AVX] 加载向量1
    ...
    for (i = 0; i < columnlength/vector_len; i++) {  /*loop with vector granularity*/
        ...
        if(vector_len<nBlockWidth) {
        ...
        } else if(vector_len<nBlockWidth * 4) {
            yfsSum3[0] = _mm256_setzero_ps();
            for(m=0; m<cntBlock; ++m){
                //first, load vectors from columns//
                yfsLoad1[0] = _mm256_loadu_ps(p1);    // [AVX] 加载l_extendedprice
                yfsLoad2[0] = _mm256_loadu_ps(p2);    // [AVX] 加载l_quantity
                yfsSum1[0] = _mm256_mul_ps(yfsLoad1[0], yfsLoad2[0]);// [AVX] 单精浮点紧缩加法
                //            _mm256_store_ps(p4,yfsSum1);
                yfsLoad3[0] = _mm256_loadu_ps(p3);   // [AVX] 加载l_tax
                yfsLoad4[0] = _mm256_add_ps(yfsSum_plus_1, yfsLoad3[0]);// [AVX] 单精浮点紧缩加法执行1+l_tax
                //            _mm256_store_ps(p5,yfsLoad4);
                //             yfsSum2 = _mm256_mul_ps(_mm256_load_ps(p4), _mm256_load_ps(p5));// [AVX] 单精浮点紧缩加法
                yfsSum2[0] = _mm256_mul_ps(yfsSum1[0], yfsLoad4[0]);// [AVX] 单精浮点紧缩加法
                //            _mm256_store_ps(p6,yfsSum2);
                //            yfsSum3 = _mm256_add_ps(yfsSum3, _mm256_load_ps(p6));
                yfsSum3[0] = _mm256_add_ps(yfsSum3[0], yfsSum2[0]);
                p1 += nBlockWidth;
                p2 += nBlockWidth;
                p3 += nBlockWidth;
                p4 += nBlockWidth;
            ...
            //aggregate vector results//
            //                q = (const data_t*)&yfsSum3;
            _mm256_storeu_ps((float*)q, yfsSum3[0]);
            result += q[0] + q[1] + q[2] + q[3] + q[4] + q[5] + q[6] + q[7];
            for(n=0; n<cntRem; ++n){
                result +=(*p1)*(*p2)*(1+(*p3));
                p1++;p2++;p3++;
            ...
            yfsSum3[0] = _mm256_setzero_ps();
            yfsSum3[1] = _mm256_setzero_ps();
            yfsSum3[2] = _mm256_setzero_ps();
            yfsSum3[3] = _mm256_setzero_ps();
            for(m=0; m<cntBlock; m=m+4){
                //first, load vectors from columns//
                yfsLoad1[0] = _mm256_loadu_ps(p1);    // [AVX] 加载l_extendedprice
                yfsLoad1[1] = _mm256_loadu_ps(p1 + nBlockWidth);
                yfsLoad1[2] = _mm256_loadu_ps(p1 + 2 * nBlockWidth);
                yfsLoad1[3] = _mm256_loadu_ps(p1 + 3 * nBlockWidth);
                yfsLoad2[0] = _mm256_loadu_ps(p2);    // [AVX] 加载l_quantity
                yfsLoad2[1] = _mm256_loadu_ps(p2 + nBlockWidth);
                yfsLoad2[2] = _mm256_loadu_ps(p2 + 2 * nBlockWidth);
                yfsLoad2[3] = _mm256_loadu_ps(p2 + 3 * nBlockWidth);
                yfsSum1[0] = _mm256_mul_ps(yfsLoad1[0], yfsLoad2[0]);// [AVX] 单精浮点紧缩加法
                yfsSum1[1] = _mm256_mul_ps(yfsLoad1[1], yfsLoad2[1]);
                yfsSum1[2] = _mm256_mul_ps(yfsLoad1[2], yfsLoad2[2]);
                yfsSum1[3] = _mm256_mul_ps(yfsLoad1[3], yfsLoad2[3]);
                //            _mm256_store_ps(p4,yfsSum1);
                yfsLoad3[0] = _mm256_loadu_ps(p3);   // [AVX] 加载l_tax
                yfsLoad3[1] = _mm256_loadu_ps(p3 + nBlockWidth);
                yfsLoad3[2] = _mm256_loadu_ps(p3 + 2 * nBlockWidth);
                yfsLoad3[3] = _mm256_loadu_ps(p3 + 3 * nBlockWidth);
                yfsLoad4[0] = _mm256_add_ps(yfsSum_plus_1, yfsLoad3[0]);// [AVX] 单精浮点紧缩加法执行1+l_tax
                yfsLoad4[1] = _mm256_add_ps(yfsSum_plus_1, yfsLoad3[1]);
                yfsLoad4[2] = _mm256_add_ps(yfsSum_plus_1, yfsLoad3[2]);
                yfsLoad4[3] = _mm256_add_ps(yfsSum_plus_1, yfsLoad3[3]);
                //            _mm256_store_ps(p5,yfsLoad4);
                //             yfsSum2 = _mm256_mul_ps(_mm256_load_ps(p4), _mm256_load_ps(p5));// [AVX] 单精浮点紧缩加法
                yfsSum2[0] = _mm256_mul_ps(yfsSum1[0], yfsLoad4[0]);// [AVX] 单精浮点紧缩加法
                yfsSum2[1] = _mm256_mul_ps(yfsSum1[1], yfsLoad4[1]);
                yfsSum2[2] = _mm256_mul_ps(yfsSum1[2], yfsLoad4[2]);
                yfsSum2[3] = _mm256_mul_ps(yfsSum1[3], yfsLoad4[3]);
                //            _mm256_store_ps(p6,yfsSum2);
                //            yfsSum3 = _mm256_add_ps(yfsSum3, _mm256_load_ps(p6));
                yfsSum3[0] = _mm256_add_ps(yfsSum3[0], yfsSum2[0]);
                yfsSum3[1] = _mm256_add_ps(yfsSum3[1], yfsSum2[1]);
                yfsSum3[2] = _mm256_add_ps(yfsSum3[2], yfsSum2[2]);
                yfsSum3[3] = _mm256_add_ps(yfsSum3[3], yfsSum2[3]);
                p1 += nBlockWidth;
                p2 += nBlockWidth;
                p3 += nBlockWidth;
                p4 += nBlockWidth;
            ...
            _mm256_storeu_ps((float*)q, yfsSum3[0]);
            result += q[0] + q[1] + q[2] + q[3] + q[4] + q[5] + q[6] + q[7];
            _mm256_storeu_ps((float*)q, yfsSum3[1]);
            result += q[0] + q[1] + q[2] + q[3] + q[4] + q[5] + q[6] + q[7];
            _mm256_storeu_ps((float*)q, yfsSum3[2]);
            result += q[0] + q[1] + q[2] + q[3] + q[4] + q[5] + q[6] + q[7];
            _mm256_storeu_ps((float*)q, yfsSum3[3]);
            result += q[0] + q[1] + q[2] + q[3] + q[4] + q[5] + q[6] + q[7];
            for(n=0; n<cntRem; ++n){
                result +=(*p1)*(*p2)*(1+(*p3));
                p1++;p2++;p3++;
