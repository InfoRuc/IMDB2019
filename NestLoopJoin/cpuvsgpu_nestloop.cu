...
typedef struct {
  sample_t *dev_data, *dev_vector;
  float *results;
  float *dev_results[nstream];
  int vec_len, data_len;
  cudaStream_t streams[nstream];
  uint64_t timer;
} device_args_t;

void generator(sample_t *fk_table, int fk_len, sample_t *pk_table, int pk_len) {
  int i, j;
  for (i = 0; i < fk_len; i++) {
    fk_table[i].rowkey = i;
...
void test(sample_t *data, sample_t *vector, int n, int vec_len, int nthreads);
//long run(int *data, int *vector, int n);
double run(sample_t *data, sample_t *vector, int n, int vec_len);
//long run_on_gpu(int *data, int n, int *vector, int vec_len, bool hasTransferTime);
double run_on_gpu(sample_t *data, int n, sample_t *vector, int vec_len, bool hasTransferTime);
...
int main(int argc, char **argv)
{
    ...
    for (step = 10; step <= 100; step+=10){
      ...
      generator(fk_table, fk_len, pk_table, pk_len);
		  printf("%4.2lf\t", run(fk_table,pk_table,MAXELEMS, pk_len));
      free(pk_table);
	  }
	  printf("\n");
    ...
    for (step = 10; step <= 100; step+=10){
      ...
      sample_t* pk_table =(sample_t*)malloc(sizeof(sample_t) * pk_len);   /* Initialize primary key vector elements in vector */
      generator(fk_table, fk_len, pk_table, pk_len);
		  printf("%4.2lf\t", run_on_gpu(fk_table, MAXELEMS, pk_table, pk_len, true));
      free(pk_table);
	}
	printf("\n");
	/*vector join for L2 cache sets*/
...
void *nlj_thread(void *args) {
    ...
    sample_t *data = param->data;
    sample_t *vector = param->vector;
    ...
    for (i = 0; i < n; i++)
    {
      for (j = 0; j < vec_len; j++)
      {
        if (data[i].rowkey == vector[j].rowkey)
          result += similar(data[i].model, vector[i].model);
      }
    }
...
__global__ void cuda_nlj_thread(sample_t *data, sample_t *vector, int n, int
    vec_len, float *CountResult) {
  // used to save vector data
    __shared__ bigint shared[THREAD_NUM];
    __shared__ float rst_shared[THREAD_NUM];
  //  int result = 0;
    ...
    for (j = 0; j < vec_block_num; j++) {
      ...
      if (tid < tid_limit)
        shared[tid] = vector[THREAD_NUM * j + tid].rowkey; 
      ...
      for (i = bid * THREAD_NUM + tid; i < n; i += BLOCK_NUM * THREAD_NUM) {
        ...
        for (k = 0; k < tid_limit; k++) {
          if (data[i].rowkey == shared[k]) {
              rst_shared[tid] += similar(data[i].model, vector[k].model);
//                rst_shared[tid] += 1;
          }
        }
      }
...
double run_on_gpu(sample_t *data, int n, sample_t *vector, int vec_len, bool hasTransferTime) {
    ...
    int dataRemSize = n;
    sample_t *tmpPtr = data;

    results = (float*)malloc(sizeof(float) * BLOCK_NUM);
    ...
    for (i = 0; i < cudaDeviceNum; i++) {
      ...
      //deviceArgs[i].results = (float*)malloc(sizeof(int) * BLOCK_NUM);
      checkCuda(cudaMalloc((void**)&deviceArgs[i].dev_data, sizeof(sample_t) * dataSize));
      checkCuda(cudaMalloc((void**)&deviceArgs[i].dev_vector, sizeof(sample_t) * vec_len));
    
      for (j = 0; j < nstream; j++) {
        checkCuda(cudaMalloc((void**)&deviceArgs[i].dev_results[j],
              sizeof(float) * BLOCK_NUM));
      }
    }
    ...
    for (i = 0; i < cudaDeviceNum; i++) {
      ...
      checkCuda(cudaMemcpyAsync(deviceArgs[i].dev_data, tmpPtr, sizeof(sample_t)
            * dataSize, cudaMemcpyHostToDevice, copyStreams[i])); 
      tmpPtr += dataSize;

      // Time for vector transfering and GPU computing
      startTimer(&timer2);
      checkCuda(cudaMemcpyAsync(deviceArgs[i].dev_vector, vector,
            sizeof(sample_t)
  ...
  //      printf("tmp data len:%d\n", tmpDataLen);
        ...
        cuda_nlj_thread<<<BLOCK_NUM, THREAD_NUM, 2 * THREAD_NUM * sizeof(float),
    ...
    for (i = 0; i < cudaDeviceNum; i++) {
      for (j = 0; j < nstream; j++) {
        checkCuda(cudaMemcpy(results, deviceArgs[i].dev_results[j],
              sizeof(float) * BLOCK_NUM, cudaMemcpyDeviceToHost));
